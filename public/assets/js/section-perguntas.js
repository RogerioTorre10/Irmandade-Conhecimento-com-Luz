/* section-perguntas.js — versão corrigida com sincronização */
(function () {
  'use strict';
  ...
  // === GUARD: verifica se veio do card ===
  if (!window.JC?.flags?.cardConfirmed) {
  ...
  async function startQA(root) {
    ...
    try {
      // === ESPERA JORNADA_BLOCKS ESTAR PRONTO ===
      if (!window.JORNADA_BLOCKS || window.JORNADA_BLOCKS.length === 0) {
        log('JORNADA_BLOCKS vazio. Aguardando loadDynamicBlocks...');
        if (window.JPaperQA?.loadDynamicBlocks) {
          await window.JPaperQA.loadDynamicBlocks();
        } else {
          throw new Error('JPaperQA.loadDynamicBlocks não disponível');
        }
      }

      if (!window.JORNADA_BLOCKS || window.JORNADA_BLOCKS.length === 0) {
        throw new Error('JORNADA_BLOCKS ainda vazio após loadDynamicBlocks');
      }

      log('JORNADA_BLOCKS carregado com', window.JORNADA_BLOCKS.length, 'blocos');

      // === Agora sim, tenta montar ===
      if (window.JPaperQA?.mount) {
        log('Usando JPaperQA.mount');
        await window.JPaperQA.mount(root, opts);
      } else if (window.JPaperQA?.start) {
        log('Usando JPaperQA.start');
        await window.JPaperQA.start(opts);
      } else {
        warn('API do JPaperQA não encontrada. Disparando evento qa:start.');
  ...
  // === EVENTO PRINCIPAL: sectionLoaded ===
  ...
  log('section-perguntas.js carregado e corrigido');
})();
