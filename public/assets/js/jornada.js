/* =========================
 Jornada Essencial (v9 estável)
 - Intro + senha "iniciar"
 - Perguntas com salvamento em localStorage
 - Botões: Enviar (gera PDF+HQ via backend), Revisar, Limpar respostas
 - Ao finalizar, baixa PDF e HQ (sequencial), mostra status e redireciona
ao início
 - Sem dependência de bundler/build
========================= */
const CONFIG = {
API_BASE: 'https://conhecimento-com-luz-api.onrender.com',
PASSWORD: 'iniciar', // use a senha que você definiu
...
// ========================= Perguntas (exemplo enxuto)
=========================
// Substitua/expanda conforme seu questionário oficial.
const QUESTIONS = [
{ id: 'q1', label: 'Quem é você hoje? (uma frase que te represente)',
type: 'text' },
{ id: 'q2', label: 'Qual foi a maior superação da sua vida?', type:
'textarea' },
{ id: 'q3', label: 'O que você quer transformar nos próximos 90 dias?',
type: 'text' }
];
// ========================= Utilitários =========================
...
if (q.type === 'textarea') {
return `<textarea class="input" rows="4" data-id="${q.id}">${v}</
textarea>`;
}
...
app.innerHTML = `
 <section class="card">
 <h1 class="text-2xl font-bold">Jornada Conhecimento com Luz ·
Essencial</h1>
<p class="small mt-2">Esta é uma jornada pessoal, simbólica e
inspiradora. Ao final, você poderá baixar um PDF com a devolutiva.</p>
 <div class="mt-6 space-y-3">
 <label class="small">Senha de acesso</label>
 <input id="pwd" class="input" type="password" placeholder="Digite a
senha e clique em Iniciar">
...
${items}
 ...
 <button id="btnClear" class="btn" style="background:#334155">Limpar
respostas</button>
 </div>
 </section>
 `;
...
const list = QUESTIONS.map(q => `
 ...
 <div class="p-3 rounded bg-slate-900/60 border border-white/5">$
...
${list}
 ...
 <div class="flex flex-wrap gap-3 pt-2">
 <button id="btnBack" class="btn" style="background:#334155">Voltar</
button>
...
// ========================= Envio ao backend + download
=========================
async function sendAndDownload() {
const el = $('#status');
el.textContent = 'Gerando sua devolutiva (PDF e HQ)...';
// 1) Envia respostas para gerar PDF
try {
const payload = { answers: state.answers, meta: { version:
CONFIG.VERSION, ts: new Date().toISOString() } };
...
el.textContent = 'Não foi possível gerar o PDF. Tente novamente em
instantes.';
console.error(e);
return;
}
...
el.textContent =
'PDF ok, mas houve falha ao gerar a HQ. Você pode tentar novamente mais
tarde.';
console.error(e);
}
// 3) Limpa dados e redireciona
clearLocal();
state.step = 'done';
render();
