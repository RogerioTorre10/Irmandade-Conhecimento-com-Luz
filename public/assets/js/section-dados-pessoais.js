...
(function () {
 ...
 (function protegerSelectsMobileDadosPessoais() {
  ...
  document.addEventListener('click', (ev) => {
    const root = document.getElementById('section-dados-pessoais');
    if (!root || !root.contains(ev.target)) return;

    const campo = ev.target.closest?.(
      'select, .select-trigger, .dropdown-trigger, [data-select-trigger], .campo-opcao, .choice, button[data-choice], [data-option]'
    );

    if (!campo) return;

    // Se o dedo arrastou, era rolagem: bloqueia abertura
    if (touchMoved) {
      ev.preventDefault();
      ev.stopPropagation();
      touchMoved = false;
    ...
    const wrapper = campo.closest?.('[data-field], .form-group, .dp-field, .campo, .select-wrap') || campo;
    const input = wrapper.querySelector?.('input, select, textarea');
    const valorAtual =
      input?.value ||
      wrapper.dataset?.value ||
      campo.dataset?.value ||
      campo.getAttribute?.('aria-selected') ||
      '';

    const jaTemValor = String(valorAtual || '').trim().length > 0;
    ...
    // Se já tem valor, só permite abrir com duplo toque
    if (!isDoubleTap) {
      ev.preventDefault();
      ev.stopPropagation();
