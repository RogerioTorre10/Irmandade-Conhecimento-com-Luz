...
// Seções que NÃO devem ser retomadas diretamente (reiniciam do início ou da senha)
...
try { await window.i18n.waitForReady(timeoutMs); return true; } catch {}
}
...
if (text) { el.dataset.text = text; el.dataset.fullText = text; }
});
...
} catch (e) { console.warn('[GUIA THEME] Não consegui ler jornada.guia:', e); }
})();
...
const voiceKind =
el.dataset.voiceKind || root?.dataset?.voiceKind ||
...
if (next) { JC.show(next); } else { window.location.href = '/termos'; }
});
...
else if (Date.now() - start > timeout) { clearInterval(interval); reject(new Error(`Timeout esperando ${selector}`)); }
}, 80);
});
}
...
if (idx >= 0 && idx < sectionOrder.length - 1) { show(sectionOrder[idx + 1]); }
} finally {
...
} catch (err) { console.warn('[JC] Falha ao reprocessar intro após idioma:', err); }
});
...
if (next) show(next); else window.location.href = '/termos';
}
...
// Retorna a seção mais avançada entre remota e local, ignorando seções proibidas
function _melhorSecaoRetomada(secaoRemota, secaoLocal) {
if (!secaoRemota && !secaoLocal) return null;
if (!secaoRemota) return SECOES_IGNORADAS_RESTORE.includes(secaoLocal) ? null : secaoLocal;
if (!secaoLocal) return SECOES_IGNORADAS_RESTORE.includes(secaoRemota) ? null : secaoRemota;
const idxRemoto = sectionOrder.indexOf(secaoRemota);
const idxLocal = sectionOrder.indexOf(secaoLocal);
const melhor = (idxRemoto >= idxLocal) ? secaoRemota : secaoLocal;
return SECOES_IGNORADAS_RESTORE.includes(melhor) ? null : melhor;
}
...
if (!lastRun) { sessionStorage.setItem('JORNADA_RUN_ID', runId); }
if (!lastRun && !hasSavedJourney) {
const keys = [
'JORNADA_GUIA', 'JORNADA_SELFIECARD', 'SELFIE_CARD',
'__SELFIECARD_DONE__', 'JORNADA_PROGRESS', 'JORNADA_RESPOSTAS', 'JORNADA_STATE_CACHE'
];
