/* /assets/js/section-perguntas.js — v2.3
   - Usa window.JPaperQA como API oficial
   - Mantém fallback leve via qa:start
...
(function () {
  ...
  // --------------------------------------
  // Helper: aguarda fim de transições
  // --------------------------------------
  ...
  // --------------------------------------
  // Descobrir API (JPaperQA é o padrão)
  // --------------------------------------
  ...
  // -------------------------------
  // Inicializa o bloco de perguntas
  // -------------------------------
  async function startQA(root) {
    ...
    const opts = {
      ...
      onComplete: (result) => {
        try {
          ...
          log('QA finalizado e salvo em __QA_ANSWERS__/__QA_META__.');
          window.toast?.('Jornada de perguntas concluída!');
          goNext();
        } catch (e) {
          err('Erro ao processar resultado do QA:', e);
          window.toast?.('Falha ao concluir as perguntas.');
        } finally {
          State.running = false;
        }
      }
    ...
    try {
      ...
      if (api && typeof api.loadDynamicBlocks === 'function') {
        log('Usando window.JPaperQA.loadDynamicBlocks');
        await api.loadDynamicBlocks(root, opts);
      } else if (api && typeof api.mount === 'function') {
        log('Usando window.JPaperQA.mount');
        await api.mount(root, opts);
      } else if (api && typeof api.start === 'function') {
        log('Usando window.JPaperQA.start');
        await api.start(opts);
      } else if (api && typeof api.run === 'function') {
        log('Usando window.JPaperQA.run');
        await api.run(opts);
      } else if (api && typeof api.init === 'function') {
        log('Usando window.JPaperQA.init(+begin)');
        await api.init(opts);
        if (typeof api.begin === 'function') await api.begin();
      } else {
        log('Fluxo padrão via evento qa:start (sem API direta)');
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (sec && !State.mounted) bindSection(sec);
  });
