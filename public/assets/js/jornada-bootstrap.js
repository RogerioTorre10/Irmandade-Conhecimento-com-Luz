(function () { 
  'use strict';
              ...
              async function waitForJC(timeout = 10000) { 
                ...
                while (!window.JC && Date.now() - start <
                       timeout) {
                  await new Promise(resolve =>
                    setTimeout(resolve, 100));
                }
                if (!window.JC) { 
                  console.error('[BOOT] JC not available after
                                timeout');
                    return false;
                } 
              ...
              async function init() {
                console.log('[BOOT] Iniciando sequência de
                            inicialização...'); 
              ...
              if (!jcAvailable) { 
                console.error('[BOOT] Aborting: JC not
                              available'); 
                  return;
              } 
              ...
              // Inicia a jornada
              console.log('[BOOT] Iniciando jornada com
                          section-intro...');
              ...
              try {
                ...
                console.error('[BOOT] Error during
                              initialization:', err);
                  }
             })(); 
