;(function () {
  ...
  // Não apaga o que já existe no canvas
  function ensureCanvas() {
    ...
    if (!content) {
      ...
      // NÃO fazemos root.innerHTML = "" aqui — preserva elementos já montados
  ...
  // Define blocks só se ainda não existir — evita sobrescrever em reimports
  if (!Array.isArray(window.JORNADA_BLOCKS)) {
  ...
  window.JORNADA_FINAL_VIDEO = window.JORNADA_FINAL_VIDEO || "/assets/img/filme-5-fim-da-jornada.mp4";

  // ---------- QA STANDALONE (opcional p/ debug) ----------
  ...
  function mount(containerId = CFG.CONTENT_ID, questions = [], { onBack, onFinish } = {}) {
    // ⚠️ Use apenas em modo standalone (fora do controller)!
    set("h");
